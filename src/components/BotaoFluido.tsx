...
type Props = {
  ...
  target?: string
  rel?: string
  /** true: só visual (renderiza <span>), não leva a lado nenhum */
  estatico?: boolean
}
...
.bfl { position:relative; isolation:isolate; display:inline-flex; align-items:center; justify-content:center;
  ...
  font-family:"Special Gothic Expanded One:Regular", "Special Gothic Expanded One", system-ui, sans-serif; font-size:18px; line-height:1.2;
...
.bfl.bfl-estatico { cursor:default; }
/* conteúdo (ícone + texto) sempre na mesma linha */
.bfl > span { display:inline-flex; align-items:center; }
.bfl > span > img { display:inline-block; }
...
function iniciar(
  el: HTMLElement,
  cores: { base: string brilho: string },
): (() => void) | undefined {
...
export default function BotaoFluido({
  ...
  // Só cria o contexto WebGL quando o botão está perto do ecrã e liberta-o quando sai:
  // o browser só aguenta ~16 contextos ao mesmo tempo.
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([e]) => {
        if (e.isIntersecting) {
          if (!limpar) limpar = iniciar(el, CORES[variante])
        } else if (limpar) {
          limpar()
          limpar = undefined
        }
  ...
  const classes = [
    "bfl",
    `bfl-${variante}`,
    tamanho !== "normal" ? `bfl-${tamanho}` : "",
    estatico ? "bfl-estatico" : "",
    className,
  ]
    .filter(Boolean)
    .join(" ")
